# Irenaeus Chan
# May 1st, 2022
# modified by Wendy Wong 
# modifications are:
# - change the pvalue to match with our panel
# - added number of variants filtered at each step
# - make file paths as input arguments
...
# Take input arguments for file paths
args <- commandArgs(trailingOnly = TRUE)

if (length(args) == 0) {
  stop("No arguments provided. Usage: Rscript PostPipelineFilters.R  bolton_bick_vars cosmic_hotspots TSG_list gene_list pd_table Final")
}

bolton_bick_vars <- args[1]
cosmic_hotspots <- args[2]
TSG_list <-  args[3]
gene_list <- args[4]
pd_table <- args[5]
Final <- args[6]
...
FilterSequencingArtifacts <- function(df) {
  ...
  # for REBC this is 0.05/8694448 = 5.750796e-9
  #pon_pvalue = 2.114164905e-6        # Archer Panel
  #pon_pvalue = 8.85582713e-7        # MGI Pilot
  pon_pvalue = 5.750796e-9 #0.05/8694448
...
unfiltered <- fread(Final)

cat (paste("starting number of variants ", nrow(unfiltered), "\n") )
